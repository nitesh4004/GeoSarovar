...
with st.sidebar:
    ...
    app_mode = st.radio(
        ...
        ["📍 RWH Site Suitability", "⚠️ Encroachment Detection"],
    ...
    elif app_mode == "⚠️ Encroachment Detection":
        st.markdown("### 3. Comparison Years")
        st.info("Detect water loss (pixels that turned from Water -> Land).")
        col1, col2 = st.columns(2)
        hist_year = col1.number_input("Historic Year", 2015, 2023, 2016)
        curr_year = col2.number_input("Current Year", 2017, 2025, 2024)
        
        params = {'hist_year': hist_year, 'curr_year': curr_year}
...
if not st.session_state['calculated']:
    ...
    m.add_tile_layer(url="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}", name="Esri Satellite", attribution="Esri")
    ...
    # ==========================================
    elif mode == "⚠️ Encroachment Detection":
        with st.spinner("Detecting Water Loss..."):
